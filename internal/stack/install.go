...
func chownToRealUser(dir string, log func(string)) {
	...
	for _, sub := range []string{"", "data", "backups"} {
		p := filepath.Join(dir, sub)
		cmd := exec.Command("chown", u.Uid+":"+u.Gid, p) // #nosec G702 G204
...
func copyFile(src, dst string) error {
	in, err := os.Open(src) // #nosec G304 G703 — src is always a known catalog cache or custom stack path
	if err != nil {
		return err
	}
	defer in.Close()

	out, err := os.OpenFile(dst, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0o640) // #nosec G302 G304 -- dst is generated by filepath.Join, not user input; 0640 is intentional for group read
	if err != nil {
		return err
	}
	defer out.Close()
