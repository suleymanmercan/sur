...
## Hibrit Task Yükleme (Hybrid Merging)
...
`sur` başlatıldığında, task'lar şu öncelik sırasına göre yüklenir ve birleştirilir (merge):
...
> [!TIP]
> Eğer yerel sistemdeki bir task'ın `id` değeri gömülü olan bir task ile eşleşirse, yerel sistemdeki task **gömülü olanın üzerine yazar (override eder)**. Bu sayede gömülü gelen varsayılan kuralları binary'yi değiştirmeden özelleştirebilirsiniz.

Ayrıca `--tasks <dizin_yolu>` parametresi ile sadece belirteceğiniz özel bir klasördeki task'ların yüklenmesini sağlayabilirsiniz.
